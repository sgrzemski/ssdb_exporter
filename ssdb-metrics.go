...
func NewSSDBExporter() *SSDBExporter {
	return &SSDBExporter{
		...
		metrics: map[string]SSDBMetric{
			"db_size": SSDBMetric{
				desc:    prometheus.NewDesc(prometheus.BuildFQName(namespace, "", "db_size"), "数据库估计的大小(可能和硬盘占用差异非常大, 以 du 命令显示的为准), 字节数. 如果服务器开启了压缩, 这个大小是压缩后的大小.", []string{"addr"}, nil),
				valType: prometheus.GaugeValue,
			},
			"links": SSDBMetric{
				desc:    prometheus.NewDesc(prometheus.BuildFQName(namespace, "", "links"), "当前服务器的连接数.", []string{"addr"}, nil),
				valType: prometheus.CounterValue,
			},
			"command_call_total": SSDBMetric{
				desc:    prometheus.NewDesc(prometheus.BuildFQName(namespace, "", "command_call_total"), "命令执行次数.", []string{"addr", "command"}, nil),
				valType: prometheus.CounterValue,
			},
			"command_time_wait_total": SSDBMetric{
				desc:    prometheus.NewDesc(prometheus.BuildFQName(namespace, "", "command_time_wait_total"), "命令等待处理的时间.", []string{"addr", "command"}, nil),
				valType: prometheus.CounterValue,
			},
			"command_time_proc_total": SSDBMetric{
				desc:    prometheus.NewDesc(prometheus.BuildFQName(namespace, "", "command_time_proc_total"), "命令执行消耗的时间.", []string{"addr", "command"}, nil),
				valType: prometheus.CounterValue,
			},
		},
	}
